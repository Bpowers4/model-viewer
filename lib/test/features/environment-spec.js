...
suite('ModelViewerElementBase with EnvironmentMixin', () => {
    ...
    suite('exposure', () => {
        ...
        test('changes the tone mapping exposure of the renderer', async () => {
            const originalToneMappingExposure = scene.renderer.threeRenderer.toneMappingExposure;
            element.exposure = 2.0;
            await timePasses();
            scene.renderer.render(performance.now());
            const newToneMappingExposure = scene.renderer.threeRenderer.toneMappingExposure;
